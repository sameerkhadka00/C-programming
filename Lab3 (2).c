...
int cuts(int n) {  // function to calculate the number of pieces after n cuts
    if (n == 0) {  // base case: 0 cuts results in 1 piece
        return 1;
    } else {  // recursive case: n cuts results in 2 times the number of pieces after n-1 cuts
        return 2 * cuts(n - 1);
    }
}

int main() {
    int n, pieces;  // n is the number of cuts, pieces is the number of pieces

    printf("Enter the number of cuts: ");
    scanf("%d", &n);  // read the number of cuts from the user

    pieces = cuts(n);  // calculate the number of pieces

    printf("After %d cuts, there will be %d pieces.\n", n, pieces);  // print the result
